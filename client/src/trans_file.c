...
int puts_file(int server_fd, char *filename)
{
    ...
    printf("filesize = %ld\n", file_stat.st_size);
    ret = send(server_fd, &train, sizeof(int)+train.length, 0);
    ERROR_CHECK(ret, -1, "send filesize");

    //接收对方本地的文件大小，用于断点续传
    ret = recv(server_fd, &data_len, sizeof(int), 0);
    ERROR_CHECK(ret, -1, "recv");
    ret = recv(server_fd, &localsize, data_len, 0);
    ERROR_CHECK(ret, -1, "recv");
    ...
    else
    {
        ...
        ret = send(server_fd, pMap+localsize, file_stat.st_size-localsize, 0);
        if (-1 == ret)
        {
            printf("client disconnected\n");
        }
...
int gets_file(int server_fd, char *filename)
{
    ...
    //接收文件大小
    off_t filesize = 0;
    ret = recv_circle(server_fd, &data_len, sizeof(int));
    ERROR_CHECK(ret, -1, "recv filesize");    
    ret = recv_circle(server_fd, &filesize, data_len);
    ERROR_CHECK(ret, -1, "recv filesize");
    ...
    if (filesize <= THREAD_SIZE)
    {
        ...
        while (1)
        {
            ...
            ret = recv_circle(server_fd, buf, data_len);
            if (-1 == ret)
            {
                printf("client disconnected\n");
                return -1;
    ...
    else
    {
        ...
        else
        {
            ...
            printf("read from info file localsize = %ld\n", localsize);
        }
        train_t train;
        memset(&train, 0, sizeof(train));
        ...
        char *pMap = (char *)mmap(NULL, filesize, PROT_READ|PROT_WRITE,
                                  MAP_SHARED, fd, 0);
        ERROR_CHECK(pMap, MAP_FAILED, "mmap");
        ret = recv_circle_file(server_fd, pMap, filesize, localsize, fd_info);
        if (ret < filesize)
        {
            printf("disconnect\n");
        }
